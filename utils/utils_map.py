    ...
    if true_p_bar != "":
        ...
        for i, val in enumerate(sorted_values):
            ...
            if i == (len(sorted_values)-1): # largest bar
                adjust_axes(r, t, fig, axes)
    # set window title
    fig.canvas.manager.set_window_title(window_title)#根据博客进行修改  fig.canvas.set_window_title加入.manager
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            if draw_plot:
                ...
                fig = plt.gcf()
                fig.canvas.manager.set_window_title('AP ' + class_name)#根据博客进行改的，fig.canvas.set_window_title加入了.manager
