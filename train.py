...
if __name__ == "__main__":
    ...
    #                       (当Freeze_Train=False时失效)
    # ------------------------------------------------------------------#
    Init_Epoch = 0  # 原0   130轮训练中断后，将其改为了120
    Freeze_Epoch = 30
    Freeze_batch_size = 32  # 32
    # ------------------------------------------------------------------#
    #   解冻阶段训练参数
    ...
    weight_decay = 5e-4
    # ------------------------------------------------------------------#
    #   lr_decay_type   使用到的学习率下降方式，可选的有'step'、'cos'、'custom'
    # ------------------------------------------------------------------#
    lr_decay_type = 'custom'
    # ------------------------------------------------------------------#
    ...
    #   vggDCD          表示进行多尺度的vgg
    #   attention       表示额外层注意力放中间 0表示仅前两层设置注意力 1表示注意力均放在最后层   进行DCD时注意力均放入前三层的DCD后
    #   pretrained1     表示不加载整个模型网络，仅加载主干网权重(仅在modelpath为空时有效)
    #   modelpath1      表示加载了本身训练出的最优权重，2表示加载整体网络权重(最初给的)
    #   [1]             表示仅进行前三层
    #   step/custom     表示学习率下降方式，custom1表示冻结和解冻阶段全都采用custom的学习率下降
    #                   原 是将学习率进行自适应，目前是将学习率在指定轮数进行设定
    #------------------------------------------------------------------#
    save_dir = 'logs_vggDCD_unmixup_modelpath2_custom1_[1]'
    # ------------------------------------------------------------------#
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            if epoch >= Freeze_Epoch and not UnFreeze_flag and Freeze_Train:
                ...
                # -------------------------------------------------------------------#
                #   判断当前batch_size，自适应调整学习率
                #   [已经修改]      在前40%：      逐步下降至Init_lr 的 0.5 倍     2e-3 —— 1e-3
                #                  接下来的40%：  逐步下降至Init_lr 的 0.25 倍    1e-3 —— 5e-4
                #                  接下来的 10%： 逐步下降至Init_lr 的 0.05 倍    5e-4 —— 1e-4
                #                  接下来的 10%： 逐步下降至Min_lr               1e-4 —— 2e-5
                # -------------------------------------------------------------------#
                # nbs = 64
                # lr_limit_max = 1e-3 if optimizer_type == 'adam' else 5e-2
                # lr_limit_min = 3e-4 if optimizer_type == 'adam' else 5e-5
