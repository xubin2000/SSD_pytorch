...
def get_lr_scheduler(lr_decay_type, lr, min_lr, total_iters, warmup_iters_ratio=0.05, warmup_lr_ratio=0.1,
                     no_aug_iter_ratio=0.05, step_num=10, pct_start_1=0.4, pct_end_1=0.8, pct_start_2=0.9, pct_end_2=0.95, pct_start_3=0.99, pct_end_3 =1.0 ):
    ...
    def custom_lr_scheduler(lr, min_lr, total_iters, pct_start_1, pct_end_1, pct_start_2, pct_end_2, pct_start_3,
                            pct_end_3):
        def lr_lambda(epoch):
            if epoch < pct_start_1 * total_iters:
                # 第一阶段：前 40% 迭代次数，学习率从 Init_lr 逐渐减小到 Init_lr 的 0.5 倍
                return lr * (1 - epoch / (pct_start_1 * total_iters)) + min_lr * (epoch / (pct_start_1 * total_iters))
            elif epoch < pct_end_1 * total_iters:
                # 第二阶段：接下来的 40% 迭代次数，学习率从 Init_lr 的 0.5 倍 逐渐减小到 Init_lr 的 0.25 倍
                progress = (epoch - pct_start_1 * total_iters) / ((pct_end_1 - pct_start_1) * total_iters)
                return lr * (0.5 - 0.25 * progress)
            elif epoch < pct_end_2 * total_iters:
                # 第三阶段：接下来的 10% 迭代次数，学习率从 Init_lr 的 0.25 倍 逐渐减小到 Init_lr 的 0.05 倍
                progress = (epoch - pct_end_1 * total_iters) / ((pct_end_2 - pct_end_1) * total_iters)
                return lr * (0.25 - 0.2 * progress)
            else:
                # 最后 10% 迭代次数，学习率从 Init_lr 的 0.05 倍 逐渐减小到 Min_lr
                progress = (epoch - pct_end_2 * total_iters) / ((pct_end_3 - pct_end_2) * total_iters)
                return lr * (0.05 - 0.045 * progress)
    ...
    elif lr_decay_type == "custom":
        custom_params = (pct_start_1, pct_end_1, pct_start_2, pct_end_2, pct_start_3, pct_end_3)
        func = custom_lr_scheduler(lr, min_lr, total_iters, *custom_params)
    else:
